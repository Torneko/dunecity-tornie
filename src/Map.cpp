...
int applyKleshmershFireResistance(const ObjectBase* target, Uint32 bulletID, int damage) {
    ...
    if(!jerichoActive || target->getOriginalHouseID() != HOUSE_REBELS) {
        return damage;
    }

    // Kleshmersh takes half damage from every flame hit, including the
    // persistent ground-fire ticks generated by Flame Tanks and Flameposts.
    return (damage + 1) / 2;
}
            ...
            for(auto objectID : affectedGroundAndUndergroundUnits) {
                ...
                if(pObject && pObject->isAStructure()) {
                    ...
                    if(realPos.x >= topLeftCorner.x && realPos.y >= topLeftCorner.y && realPos.x < bottomRightCorner.x && realPos.y < bottomRightCorner.y) {
                        const int structureDamage = applyKleshmershFireResistance(
                            pStructure, bulletID, lround(damage));
                        pStructure->handleDamage(structureDamage, damagerID, damagerOwner);
                    ...
                    if(distance <= damageRadius) {
                        ...
                        if(bulletID == Bullet_DRocket) {
                            ...
                            scaledDamage = applyKleshmershFireResistance(
                                pUnit, bulletID, scaledDamage);
